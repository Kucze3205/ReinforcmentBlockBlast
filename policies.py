...
class LookaheadPolicy:
    ...
    **wylosowanymi** tackami, a nie przez statyczny zamiennik `placeable_shapes`.
    ...
    def __init__(self, weights=None, beam=None, samples=None, branch=None,
        ...
        self._seed = seed
        self.last_expanded = 0
        # `None` = wartość liścia z `weights · features(board)` (zachowanie
        # niezmienione). Alternatywne źródło (np. sieć N-tuple, #123) wpina się
        # przez podklasę, która nadpisuje ten atrybut — patrz `NTupleLookaheadPolicy`.
...
class NTupleLookaheadPolicy(LookaheadPolicy):
    ...
    zamiast `weights · features(board)`.
...
def _tray_beam_search(board, pieces, combo, combo_counter, weights, beam,
    ...
    - `leaf_value` — `None` (domyślnie) liczy wartość liścia jako
      `weights · features(board)` (`_weighted_features`, zachowanie od #92 bez
      zmian). Podanie funkcji `board -> float` (np. `NTupleValue.value`, #123)
      zastępuje ją tym wywołaniem; `weights` jest wtedy ignorowane.
    """
    value_fn = leaf_value if leaf_value is not None else (lambda b: _weighted_features(weights, b))
    root = {
        "board": board.copy(),
        "pieces": tuple(pieces),
        "combo": combo,
    ...
    for level in range(levels):
        ...
        for candidate in candidates:
            candidate["score"] = candidate["gain"] + value_fn(candidate["board"])
